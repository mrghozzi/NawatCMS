...
<div style="display: grid; grid-template-columns: 1fr 2fr; gap: 24px;">
    ...
    <div style="background: var(--surface); border: 1px solid var(--border); border-radius: 12px; padding: 24px; height: fit-content;">
        <h3 style="margin-top: 0; margin-bottom: 20px; font-size: 16px;">{{ __('Add New Tag') }}</h3>
        ...
        <form action="{{ route('admin.tags.store') }}" method="POST">
            ...
            <button type="submit" class="primary-action" style="width: 100%;">{{ __('Add New Tag') }}</button>
        </form>
    </div>
            ...
            <tbody>
                @forelse($tags as $tag)
                    <tr style="border-bottom: 1px solid var(--border);">
                        <td style="padding: 16px 24px; font-weight: 500;">{{ $tag->name }}</td>
                        <td style="padding: 16px 24px; color: var(--muted); font-size: 13px;">{{ $tag->slug }}</td>
                ...
                @empty
                    <tr>
                        <td colspan="3" style="padding: 32px 24px; text-align: center; color: var(--muted);">No tags found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
